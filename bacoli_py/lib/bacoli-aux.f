c Modified version of bacoli-aux.f, which removes the COLROW package,
c replacing it with LAMPACK.
...
C
C     BACOLI --> BACOLIVEC
C         Add fvec call, change work array size.
C         Note: should probably remove all calls to normal f
C               in final version.
...
c     replacing crdmcp with lamdec
c     BACOLI --> BACOLIVEC
...
c     replaced with call to lamsol
c      x = delta(1:neq)
