...
c       to use the linear system solver LAMPACK instead of COLROW.
c       additionally the calls to the user routines f and uinit have
c       been vectorized.
...
c CALJAC, CALRES, LAMSOL and LAMDEC once each (this is where the two
...
C     BACOLI --> BACOLIVEC
C         Modified array offsets for irwork.
...
c     BACOLI --> BACOLIVEC
c         Modified to allow vectorized uinit calls
     ...
     &          mflag(9), uinit, uinitvec, bndxa, difbxa, bndxb, difbxb)
...
c-----------------------------------------------------------------------
  600 continue
c      write(6,9999) 'ERROR: BACOLI runtime error in time stepping.'
c      write(6,9999) '       An error code and message should have'
c      write(6,9999) '       been issued by DASSL.'
      return
  610 continue
      if (istart .eq. 1) then
         write(6,9998) 'Remeshed ', maxrsh
     ...
     &                 , ' times at', ' t0 =', torign
      ...
      return
  620 continue
      write(6,9999) 'A singular matrix arises at the initial'
      write(6,9999) '       step. '
      idid = -42
      return
  630 continue
      write(6,9999) 'A singular matrix arises during remeshing'
      write(6,9997) '       at t0 =', rpar(ipar(idasr)-1+4)
      idid = -43
      return
  640 continue
      if (istart .eq. 1) then
         write(6,9998) 'nint >', nintmx, ' at'
     &                 , ' t0 =', rpar(ipar(idasr)-1+4)
      else
         write(6,9998) 'nint >', nintmx, ' at'
     &                 , ' t0 =', torign
...
c-----------------------------------------------------------------------
  ...
  710 continue
      write(6,9999) 'RuntimeInput violation.'
      write(6,9999) 'Require:  0 <= mflag(i) <= 1, i = 1, 2, ..., 8.'
      idid = -51
      return
  711 continue
      write(6,9999) 'Input violation.'
...
c     Define the collocation point sequence.
   ...
   50 continue
     ...
     &                ifgfdj, uinit, uinitvec, bndxa, difbxa, bndxb,
     &                difbxb)
...
c                               lamdec overwrites the input collocation
c                               matrix.
c
        integer                 idelta
...
c     BACOLI --> BACOLIVEC
c         Moved into vectorized computation
...
c     BACOLI --> BACOLIVEC
      call lamdec(neq,work(iabdtp),npde,2*npde,work(iabdbk),kcol*npde,
...
c     conditions, i.e. y(t0). If not, this gives the predictor of y(t0).
      call dcopy(neq, y, 1, x, 1)
      call lamsol(work(iabdtp),npde,2*npde,work(iabdbk),kcol*npde,
     &            (kcol+nconti)*npde,nint,work(iabdbt),npde,ipivot,x,y)
c     BACOLI --> BACOLIVEC
...
c     Solve the corrector equation.
      call dcopy(neq, work(idelta:idelta+neq-1), 1, x, 1)
c      x = work(idelta:idelta+neq-1)
      call lamsol(work(iabdtp),npde,2*npde,work(iabdbk),kcol*npde,
     &            (kcol+nconti)*npde,nint,work(iabdbt),npde,ipivot,
     &            x,work(idelta))
c     BACOLI --> BACOLIVEC
...
c                               BACOLI --> BACOLIVEC
c                               changed to accomidate vectorization.
c                               lw >= 2*npde*npde*nconti+
c                                     npde*npde*kcol*(kcol+nconti)*nint+
c                                     npde*3+2*npde*npde+npde+
c                                     6*npde*(kcol*nint)
...
c
        integer                 vnpts
c                               BACOLI --> BACOLIVEC
c                               number of collocation points vectorized
c                               f user routine must be used on.
c                               vnpts = nint*kcol
c
        integer                 voffset
c                               BACOLI --> BACOLIVEC
...
c     BACOLI --> BACOLIVEC
c     Modified offsets into work array to allow vectorization.
      iu     = iabdbt + npde*npde*nconti
      iux    = iu     + npde
      iuxx   = iux    + npde

c     BACOLI --> BACOLIVEC
c     added swap memory for iu, iux, iuxx for vectorized calls.
      swapiu = iuxx    + npde
...
c           BACOLI --> BACOLIVEC
c           set offset into vectorized work arrays.
c            voffset = npde * (j - 1 + kcol*(i-1))
c           New approach:
...
c           derivatives at the current collocation point.
c           BACOLI --> BACOLIVEC
c           Modified call to assign to offsets in the work array.
c           Note: eval is a candidate for vectorization.
c            call eval(npde,kcol,ii,jj,ncpts,work(iu+voffset),
c     &                work(iux+voffset),work(iuxx+voffset),
c     &                fbasis(1,1,jj),y)
            call eval(npde,kcol,ii,jj,ncpts,work(iu),
     &                work(iux),work(iuxx),
     &                fbasis(1,1,jj),y)
c           BACOLI --> BACOLIVEC
c           Put values in proper place in swap array
            do 181 k = 1, npde
...
c     BACOLI --> BACOLIVEC
      vnpts = nint * kcol
      call fvec(t0, xcol(2), work(swapiu), work(swapiux),
     &       work(swapiuxx), yprime(npde+1), vnpts, npde)
...
c     replaced with call to lamdec
      call lamdec(neq,work(iabdtp),npde,2*npde,work(iabdbk),kcol*npde,
     &            (kcol+nconti)*npde,nint,work(iabdbt),npde,ipivot,
     &            icflag)
c     BACOLI --> BACOLIVEC
...
c     coefficients for the initial conditions, i.e. y(t0).
      do 90 i = 1, nstep
         ii = (i - 1) * neq + 1
c        BACOLI --> BACOLIVEC
c         x_in = y(ii:ii+neq-1)
         call dcopy(neq, y(ii:ii+neq-1), 1, x_in, 1)
         call lamsol(work(iabdtp),npde,2*npde,work(iabdbk),kcol*npde,
...
c-----------------------------------------------------------------------
c     BACOLI --> BACOLIVEC
c         allow use of vectorized user routines.
        implicit none
c Constants:
        integer                 nconti
        parameter              (nconti = 2)
...
c
c       Work storage:
        double precision        work(3*npde*kcol*nint)
c                               work is a floating point work array
...
c     &                work(iux+voffset),work(iuxx+voffset),fbasis(kk),y)
            call eval(npde,kcol,ii,jj,ncpts,work(iu),
     &                work(iux),work(iuxx),fbasis(kk),y)
...
c     &              work(iuxx), delta(mm), npde)

   20    continue
   30 continue

      vnpts = nint * kcol
      call fvec(t, xcol(2), work(swapiu), work(swapiux), work(swapiuxx),
     &       delta(npde+1), vnpts, npde)
...
cc     &                -1)
c
c              delta(npde*i+j)=work(swapiu+(j-1)*nint*kcol+i
c     &                -1)
c  210     continue
c  200 continue
